...
with open('chat.txt', 'r', encoding='utf-8') as corpus_file:
    ...
    sentences = itertools.chain(
        *[nltk.sent_tokenize(x.lower()) for x in lines])
...
try:
    print("Loading model...")
    load_model_parameters_theano(savefile, model)
except Exception as e:
    print("Failed. Training model...")
    losses = train_with_sgd(
        model, X_train[:100], y_train[:100], nepoch=100, evaluate_loss_after=1)
    save_model_parameters_theano(savefile, model)
...
def generate_sentence(model):
    ...
    while not new_sentence[-1] == word_to_index[sentence_end_token]:
        ...
        while sampled_word == word_to_index[unknown_token] or sampled_word > len(index_to_word) -1:
